...
start_link() ->
    ...
    erli_worker_sup:start_link().

-spec start() -> ok.
start() ->
    ok = ensure_deps_started(),
    application:load(erli_worker), %% force loading of .config
    case application:get_env(erli_worker, unmanaged) of
	{ok, true} ->
    ...
    application:start(erli_worker).


-spec stop() -> ok.
stop() ->
    Res = application:stop(erli_worker),
