...
%%% @version 0.1a
%%% @doc Utilities for erli API workers.
...
meta_proplist(CollectionType, {Start, End}) ->
    ...
    M = [{<<"totalCollectionSize">>, Total},
	 ...
	 {<<"rangeStart">>, Start},
	 {<<"rangeEnd">>, End},
	 {<<"maxCollectionOffset">>, get_env(max_collection_offset)}],
...
to_proplist(#target{id=Id, record_number=_RN, url=Url, last_modified=LM,
    ...
    [{<<"id">>, Id},
     {<<"href">>, <<"/targets/", Id/bitstring>>},
     {<<"bannedStatus">>, B},
     {<<"flagCount">>, FC},
     {<<"lastModified">>, LM},
     {<<"rels">>,
      [{<<"targetUrl">>, Url},
       {<<"paths">>, <<"/targets/", Id/bitstring, "/paths">>},
       {<<"visits">>, <<"/targets/", Id/bitstring, "/visits">>},
       {<<"thumbnail">>, <<"/static/thumbnails/", Thumbnail/bitstring>>}]}];
to_proplist(#path{id=Id, record_number=_RN, target_id=_, is_banned=B}) ->
    [{<<"id">>, Id},
     {<<"href">>, <<"/paths/", Id/bitstring>>},
     {<<"bannedStatus">>, B},
     {<<"rels">>,
      [{<<"target">>, <<"/paths/", Id/bitstring, "/target">>},
       {<<"visits">>, <<"/paths/", Id/bitstring, "/visits">>}]}];
to_proplist(#visit{id=Id, path_id=PId, geo_location=Loc, time=Time}) ->
    [{<<"id">>, Id},
     {<<"href">>, <<"/visits/", Id/bitstring>>},
     {<<"visitTime">>, Time},
     {<<"visitorOrigin">>, Loc},
     {<<"rels">>,
      [{<<"path">>, <<"/paths/", PId/bitstring>>}]}];
...
parse_range(HeaderValue, CollectionType) ->
	...
	{match, [Unit, X, Y]} when Unit =:= M ->
	    ...
	    MaxOffset = get_env(max_collection_offset),
