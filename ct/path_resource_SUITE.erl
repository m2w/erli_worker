...
init_per_suite(Config) ->
    ...
    RootUrl = "http://localhost:" ++ PortS ++ "/paths/",
...
post_json_data(Config) ->
    ...
    Target = erli_storage:write(#target{url=TargetUrl}),
...
idempotent_calls_to_entity(Config) ->
    ...
    Path = erli_storage:write(#path{target_id=Target#target.id}),

    T = #target{url=BannedUrl, is_banned=true, flag_count=9000},
    BannedTarget = erli_storage:write(T),
    BannedPath = erli_storage:write(#path{target_id=BannedTarget#target.id,
					  is_banned=true}),
...
delete_entity(Config) ->
    ...
    AlmostBannedPath = erli_storage:write(
